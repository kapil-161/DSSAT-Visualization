...
def standardize_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(potential_numeric_cols) > 0:
        ...
        numeric_df = df[potential_numeric_cols].apply(pd.to_numeric, errors="coerce")
...
def vectorized_date_conversion(df: pd.DataFrame, date_col: str = "DATE") -> pd.DataFrame:
    ...
    if date_col in df.columns:
        ...
        df[date_col] = pd.to_datetime(year_col + doy_col, format="%Y%j", errors="coerce")
    
    return df

def handle_missing_xvar(obs_data: pd.DataFrame, x_var: str, sim_data: pd.DataFrame = None) -> pd.DataFrame:
    ...
    if "DATE" in obs_data.columns and not pd.api.types.is_datetime64_any_dtype(obs_data["DATE"]):
        obs_data = vectorized_date_conversion(obs_data)
        
    if sim_data is not None and "DATE" in sim_data.columns and not pd.api.types.is_datetime64_any_dtype(sim_data["DATE"]):
    ...
    # Try to infer from simulation data efficiently
        ...
        date_to_xvar = dict(zip(
            pd.to_datetime(sim_data["DATE"].dropna()).unique(),
    ...
    if f"{x_var}" not in obs_data.columns:
        ...
        obs_data[f"{x_var}"] = np.arange(len(obs_data))  # Faster than range()
